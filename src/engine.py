...
class generate():
    """
    This class contains all functions to generate a design.
    """
    def __init__(self, config: dict):
        self.config = config
        self.parameters = self.random_parameters()
    ...
    def random_parameters(self) -> dict:
        ...
        model = np.random.choice(self.config["models"]["type"])
        parameters["model"] = model
        
        # Generate random parameter from list or value
        for key, val in self.config["models"][model]["parameters"].items():
        ...
        parameters["phi_texture_type"] = np.random.randint(
            self.config["models"][model]["num_texture_types"])
    ...
    def ellipsoid(self, theta: np.ndarray, phi: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def torus(self, theta: np.ndarray, phi: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def grid(self, a_max: float = np.nan, b_max: float = np.nan) -> Tuple[np.ndarray, np.ndarray, mtri.triangulation.Triangulation]:
        ...
        """
        num_points = self.parameters["model"]["num_points"]
    ...
    def random_spline(self, array: np.ndarray, order_max: int = 30, knots_max: int = 7, offset: int = 0) -> np.ndarray:
    ...
    def twist(self, x: np.ndarray, y: np.ndarray, twist_frequency: float) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if rotation_flag:
            ...
            alpha = np.linspace(0, 2 * np.pi * twist_frequency,
                                self.parameters["models"]["num_points"]) * np.random.choice([-1, 1])
            # Create copys of array for every point in the grid and flatten
            alpha = np.kron(alpha, np.ones((self.parameters["models"]["num_points"], 1))).flatten()

            # Create spline transform of angle array
            if fuzzy_flag:
                alpha = self.random_spline(alpha)
    ...
    def tilt(self, x: np.ndarray, y: np.ndarray, z: np.ndarray, x_tilt: float = 1., y_tilt: float = 1.) -> Tuple[np.ndarray, np.ndarray]:
        ...
        x += x_tilt * self.random_spline(z)
        y += y_tilt * self.random_spline(z)

        return x, y


    def edginess(self, array: np.ndarray, angle: np.ndarray, edginess: float = 0.) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def modulator(self, array: np.ndarray, scaler: float = 1, offset: int = 0) -> np.ndarray:
        ...
        modulator = self.random_spline(array, offset=offset)
    ...
    def texture(self, array: np.ndarray, texture_type: int = 0, amplitude: float = 0.1, frequency: float = 1., duty_cycle: float = 0.5) -> np.ndarray:
    ...
    def angular_texture(self, theta: np.ndarray, phi: np.ndarray, parameters: dict) -> dict:
    ...
    def scale_xy(self, x: np.ndarray, y: np.ndarray, scaler: float = 1.) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def radialsymetric(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, mtri.triangulation.Triangulation]:
        ...
        theta, phi, triangles = self.grid()

        # Randomly pick ellipsoid or torus base design
        ridx = np.random.randint(2)
        if ridx == 0:
            x, y, z = self.ellipsoid(theta, phi)
        else:
            x, y, z = self.torus(theta, phi)

        # Generate angular textures
        textures = self.angular_texture(theta, phi, self.parameters)
        ...
        x, y = self.twist(
            x, y, self.parameters["models"]["rsym"]["parameters"]["e1_twist"])
        x, z = self.twist(
            x, z, self.parameters["models"]["rsym"]["parameters"]["e2_twist"])
        y, z = self.twist(
            y, z, self.parameters["models"]["rsym"]["parameters"]["e3_twist"])

        return x, y, z, triangles


    def cylindersymetric(self, model = "csym") -> Tuple[np.ndarray, np.ndarray, np.ndarray, mtri.triangulation.Triangulation]:
        ...
        parameters = self.parameters["models"][model]
        # Generate grid used in for cylindrical coordinate systems
        z, phi, triangles = self.grid(
        ...
        x, y = self.edginess(modulator, phi, parameters["edginess"])

        x, y = self.twist(x, y, parameters["twist"])

        x, y = self.tilt(x, y, z, parameters["tilt_x"], parameters["tilt_y"])

        x, y = self.scale_xy(x, y, parameters["radius"])

        return x, y, z, triangles


    def start(self, model: str = "csym") -> Tuple[np.ndarray, np.ndarray, np.ndarray, mtri.triangulation.Triangulation]:
    ...
    def get_ijk(self, triangles: mtri.triangulation.Triangulation) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def export_stl(self, path: str, x: np.ndarray, y: np.ndarray, z: np.ndarray, triangles: mtri.triangulation.Triangulation):
